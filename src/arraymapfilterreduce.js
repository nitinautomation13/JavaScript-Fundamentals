...
//I want to capture those name where length is greater than three
let longsName = names.filter(n =>n.length>3);
console.log(longsName);
...
//Give me those product which is related to the apple
let applePrducts = productData
                  .filter(p => p.startsWith('apple'))
                      .filter(p=> p.includes('iph'))
                           .map(p => p.replace('iph', 'iPhone'));

console.log(applePrducts);
...
let actAddess = empAddress.reduce((initaddress, word) => initaddress+word+",", "")
console.log(actAddess);
